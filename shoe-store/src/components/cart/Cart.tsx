...
export const Cart: React.FC = () => {
  ...
  const deleteHandle = (id, size) => {
    deleteProductInCart(id, size)
    const a = { id, size }
                ...
                <tbody>
                    {items.map((item, i) => (
                      <tr key={item.key}>
                        ...
                        <td><button className="btn btn-outline-danger btn-sm" onClick={() => { deleteHandle(item.id, item.sizes) }} >Удалить</button></td>
                      </tr>
                    ))}
                  <tr>
